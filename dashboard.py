...
if fl is not None:
    ...
    required_columns = ["Order Date", "Region", "State", "City", "Category", "Sales", "Profit", "Quantity"]
    if not all(col in df.columns for col in required_columns):
        st.error(f"Missing columns in the uploaded file. Expected columns: {', '.join(required_columns)}")
        st.stop()

    df["Order Date"] = pd.to_datetime(df["Order Date"], errors='coerce')
    df.dropna(subset=["Order Date"], inplace=True)  # Ensure only valid dates are used
    ...
    fig3 = px.treemap(df, path=["Region", "Category", "Sub-Category"], values="Sales", hover_data=["Sales"], color="Sub-Category")
    ...
    scatter_fig = px.scatter(df, x="Sales", y="Profit", size="Quantity", title="Relationship between Sales and Profit using Scatter Plot")
    st.plotly_chart(scatter_fig, use_container_width=True)

    with st.expander("View Data"):
        st.write(df.head(500).style.background_gradient(cmap=cmap))
